...
# ---------------------------------------------------------------------------
# Run configuration (single seed, paper-matching schedule)
# ---------------------------------------------------------------------------
SEQ="${SEQ:-0}"
SEED="${SEED:-1}"
GPU="${GPU:-0}"
...
# ---------------------------------------------------------------------------
# Helper: expected results-pkl path for a given detector
# ---------------------------------------------------------------------------
expected_pkl() {
    local det="$1"
...
# ---------------------------------------------------------------------------
# Helper: run a single detector with checkpoint/resume support.
#
#   run_detector  <det>  [extra_arg1 extra_arg2 ...]
#
# Checks for a completed results pkl first (skip if found), then runs
# FAME.py with --checkpoint_every so mid-training state is saved every
# CKPT_EVERY steps.  On resume FAME.py auto-detects the checkpoint file
# and continues from the last saved step.
# ---------------------------------------------------------------------------
run_detector() {
    local det="$1"; shift
    local extra=("$@")   # any additional args specific to the detector
